...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // --- 1. Total records ---
    $sqlTotal = "SELECT COUNT(*) as cnt FROM logs_sms";
    ...
    // --- 3. Filtered count ---
    $sqlFiltered = "SELECT COUNT(*) as cnt FROM logs_sms $where";
    ...
    // --- 6. Data query ---
    $sqlData = "SELECT id, data, CONCAT(country_code, ' ', mobile) AS phone, device_id, created_at
                FROM logs_sms
                $where
...
<div class="card mb-3 shadow-sm border">
    <div class="card-body py-3 px-4">
        <div class="d-flex align-items-center justify-content-between">
            <h5 class="h4 text-primary m-0 fw-bolder">SMS & Whatsapp Logs</h5>
        </div>
    </div>
</div>
...
$q1 = $conn->query("SELECT COUNT(*) AS cnt FROM logs_sms");
$stats['total'] = $q1->fetch_assoc()['cnt'];

$q2 = $conn->query("SELECT COUNT(*) AS cnt FROM logs_sms WHERE DATE(created_at) = CURDATE()");
