...
function getPassages($conn) {
    try {
        ...
        $totalSql = "SELECT COUNT(*) as total FROM reading_readallowed_questions ra LEFT JOIN reading_readallowed sc ON ra.category_id = sc.category_id $where";
        ...
        if (!empty($search)) {
            ...
            $where .= " AND (ra.passage_title LIKE '%$search%' OR ra.passage_text LIKE '%$search%' OR sc.category_name LIKE '%$search%' OR ra.pronunciation_tips LIKE '%$search%') ";
        }
        
        // Filtered count
        $filteredSql = "SELECT COUNT(*) as total FROM reading_readallowed_questions ra LEFT JOIN reading_readallowed sc ON ra.category_id = sc.category_id $where";
        ...
        // Main data query
        $sql = "SELECT ra.passage_id, ra.passage_title, ra.passage_text, ra.category_id, ra.word_count, 
                       ra.pronunciation_tips, ra.points, ra.is_active, ra.created_at, sc.category_name
                FROM reading_readallowed_questions ra 
                LEFT JOIN reading_readallowed sc ON ra.category_id = sc.category_id 
                $where 
                ORDER BY $orderBy 
                $limit";
        ...
        if ($result) {
            while ($row = $result->fetch_assoc()) {
                $data[] = [
                    ...
                    'pronunciation_tips' => $row['pronunciation_tips'] ?? '',
                    'points' => $row['points'],
...
function getPassage($conn) {
    ...
    $sql = "SELECT ra.passage_id, ra.category_id, ra.passage_title, ra.passage_text, ra.word_count,
                   ra.pronunciation_tips, ra.points, ra.is_active, ra.created_at, ra.updated_at, sc.category_name 
            FROM reading_readallowed_questions ra 
            LEFT JOIN reading_readallowed sc ON ra.category_id = sc.category_id 
            WHERE ra.passage_id = ?";
...
function addPassage($conn) {
    try {
        ...
        $pronunciation_tips = trim($_POST['pronunciation_tips'] ?? '');
        ...
        $word_count = str_word_count($passage_text);
        
        // Begin database transaction for data integrity
        $conn->begin_transaction();
        
        try {
            $sql = "INSERT INTO reading_readallowed_questions (category_id, passage_title, passage_text, word_count, pronunciation_tips, is_active, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())";
            $stmt = $conn->prepare($sql);
            if (!$stmt) {
            ...
            $stmt->bind_param("issisi", 
                $category_id, $passage_title, $passage_text, $word_count, 
                $pronunciation_tips, $is_active
            );
            ...
            error_log("Passage added successfully: ID $insertId, Category: $category_id, Words: $word_count");
...
function updatePassage($conn) {
    try {
        ...
        $sql = "UPDATE reading_readallowed_questions SET category_id=?, passage_title=?, passage_text=?, word_count=?, pronunciation_tips=?, is_active=?, updated_at=NOW() WHERE passage_id=?";
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            throw new Exception('Database prepare failed: ' . $conn->error);
        }
        $stmt->bind_param("issisii", $category_id, $passage_title, $passage_text, $word_count, $pronunciation_tips, $is_active, $passage_id);
        
        if ($stmt->execute()) {
            echo json_encode(['success' => true, 'message' => 'Passage updated successfully', 'word_count' => $word_count]);
        } else {
...
function deletePassage($conn) {
    ...
    $sql = "DELETE FROM reading_readallowed_questions WHERE passage_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $passage_id);
...
function getCategories($conn) {
    ...
    $tableCheck = $conn->query("SHOW TABLES LIKE 'reading_readallowed'");
...
function addCategory($conn) {
    ...
    try {
        ...
        if ($display_order > 0) {
            ...
            $adjustSql = "UPDATE reading_readallowed SET display_order = display_order + 1 WHERE display_order >= ?";
        ...
        $sql = "INSERT INTO reading_readallowed (category_name, category_description, display_order, points) VALUES (?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
...
function updateCategory($conn) {
    ...
    try {
        ...
        $getCurrentSql = "SELECT display_order FROM reading_readallowed WHERE category_id = ?";
        ...
        if ($new_display_order != $old_display_order && $new_display_order > 0 && $old_display_order > 0) {
            if ($new_display_order < $old_display_order) {
                ...
                $adjustSql = "UPDATE reading_readallowed SET display_order = display_order + 1 
        ...
        $sql = "UPDATE reading_readallowed SET category_name=?, category_description=?, display_order=?, points=? WHERE category_id=?";
        $stmt = $conn->prepare($sql);
...
function deleteCategory($conn) {
    ...
    $sql = "DELETE FROM reading_readallowed WHERE category_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $category_id);
...
function duplicatePassage($conn) {
    ...
    $sql = "SELECT * FROM reading_readallowed_questions WHERE passage_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $passage_id);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        $sql = "INSERT INTO reading_readallowed_questions (category_id, passage_title, passage_text, word_count, pronunciation_tips, is_active) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("issisi", 
            $row['category_id'], 
            $new_passage_title, 
...
<div class="card mb-3 shadow-sm border">
    <div class="card-body py-3 px-4 d-flex justify-content-between align-items-center">
        <h5 class="h4 text-primary fw-bolder m-0">Read Allowed - Reading Practice</h5>
    </div>
</div>
                ...
                <div class="card-body">
                    <form id="quickAddForm">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="quick_passage_text" name="passage_text" rows="4" required placeholder="Enter the passage text to read aloud..."></textarea>
                            <small class="text-muted">Word count will be calculated automatically</small>
                        </div>
                        
                        <div class="mb-3">
                            <label class="form-label">Pronunciation Tips (Optional)</label>
                            <textarea class="form-control" id="quick_pronunciation_tips" name="pronunciation_tips" rows="2" placeholder="Add pronunciation guidance..."></textarea>
                        </div>
                ...
                <form id="editPassageForm">
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="edit_pronunciation_tips" class="form-label">Pronunciation Tips (Optional)</label>
                                    <textarea class="form-control" id="edit_pronunciation_tips" name="pronunciation_tips" rows="2"></textarea>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
            ...
            categoryPassagesTable = $('#categoryPassagesTable').DataTable({
                ...
                columns: [
                    ...
                    { 
                        data: 'word_count',
                        render: function(data, type, row) {
                            return data ? data + ' words' : '0 words';
                        }
                    },
                    { 
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (result.success) {
                        ...
                        $('#edit_pronunciation_tips').val(data.pronunciation_tips || '');
