...
try {
    ...
    if (!isset($_FILES['bin_a'])) {
        throw new Exception("firmware files are required.");
    }

    if ($_FILES['bin_a']['error'] !== UPLOAD_ERR_OK) {
    ...
    $sql = "INSERT INTO firmware (product_id, version, description, type, data_a, data_b, is_deleted, created_at)
            VALUES ('$product_id', '$version', '$description', 'wifi', '$data_a', '', 0, NOW())";
    ...
    $conn->close();
    header("Location: /admin/user/firmware/wifi.php?msg=Firmware uploaded successfully");
    exit;
    ...
    header("Location: /admin/user/firmware/wifi.php?err=" . urlencode($e->getMessage()));
    exit;
}
